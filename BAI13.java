...
public class BAI13 {
    public static void main(String[] args) throws IOException{
        ...
        for(int i = 0;i<m;i++){
            String s = sc.nextLine();
            String []tmp = s.trim().split("\\s+"); //tmp[0] là id, tmp[1] là pass người đó login
            if(mp1.containsKey(tmp[0])){//Có tên trong danh sách id pass ban đầu thì mới biết log đúng hay sai
                if(mp1.get(tmp[0]).equals(tmp[1])){ //so sánh mật khẩu trong ds ban đầu với mật khẩu họ login
